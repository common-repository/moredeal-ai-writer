...
<div class="moredeal-maincol">
    <div class="wrap">
        <h2>
            <?php Plugin::translation_e( 'Moredeal AI Writer ' ) . Plugin::translation_e( "Token Recharge" ); ?>
            <?php if ( ! Plugin::is_free() ): ?>
                <span
                    class="moredeal-label moredeal-label-pro"><?php Plugin::translation_e( Plugin::plugin_level() . ' ' . Plugin::version() ) ?></span>
            <?php else: ?>
        ...
        <?php if ( ! empty( $page_slug ) ): ?>
            ...
            <!--                --><?php //AccountManager::getInstance()->render_aigc_account( 'Token' ); ?>
            <!--            </div>-->
            <form action="options.php" method="POST">
                <?php settings_fields( $page_slug ); ?>
                <?php AdminHelper::doTabsSections( $page_slug ); ?>
                <?php submit_button( Plugin::translation( 'Token Recharge' ) ); ?>
            </form>
        <?php endif; ?>
    </div>
</div>
